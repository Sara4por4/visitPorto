                    ...
                    >
                        @foreach($records as $record)
                            <tr data-id="{{ $record->id }}" id="item-{{ $record->id }}" class="ui-state-default">
                                ...
                                @if(!empty($toggleable_active) || $toggleable_type)
                                    <td>
                                        @if(!empty($toggleable_active))
                                            ...
                                            {!! Form::close() !!}
                                        @endif
                                        @if(!empty($toggleable_type))
                                                {!! Form::model($record->id, ['method' => "PATCH", 'url' => route('sgc.change_type.customer', ['user'=>$record->id]), 'class'=>'change_type_customer col-sm-6']) !!}
                                                <button type="submit" class="btn {{$record->type == 'member' ? 'btn-success' : 'btn-danger'}} btn-xs">{{ $record->type == 'member' ? 'Normal' : 'Retalho' }}</button>
                                                {!! Form::close() !!}
                                            @endif
                                            <div class="row">
                                ...
                                @if(!empty($actions['edit']) || !empty($actions['destroy']))
                                    <td class="text-right">
                                        ...
                                        @if(!empty($actions['show']))
                                            <a href="{{ action($actions['show'], ['id' => $record->id]) }}"
                                               class="btn btn-info btn-xs"><i class="fa fa-eye"></i> Ver </a>
                                        @endif
                                        <!-- End: Edit action -->
